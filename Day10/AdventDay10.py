...
def find_trailhead_score(array,x_pos,y_pos,current_number):
    ...
    # If the current position matches the current number and is 0
        ...
        array[y_pos][x_pos] = -1  # Mark the current position as visited
        return 1
...
if (__name__ == '__main__'):
    ...
    with open("input10.txt", "r") as f:
        ...
        array_copy = np.copy(array)  # Create a copy of the array for processing
        ...
        for i in range(len(zeros_in_array[0])):  # Iterate over each zero position
            ...
            final_score += find_trailhead_score(array_copy, zeros_in_array[1][i], zeros_in_array[0][i], 0)  # Calculate the score for each zero position
            array_copy = np.copy(array)  # Reset the array copy for the next iteration
